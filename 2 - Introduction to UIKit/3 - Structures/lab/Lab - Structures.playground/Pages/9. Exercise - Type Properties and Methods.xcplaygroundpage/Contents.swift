...
struct User {
    ...
    var email: String
    var age: Int
    static var currentUser = User(userName: "JJ", email: "jjspillmann01@bvsd.org", age: 17)
    static func logIn(user: User) {
        currentUser.userName = user.userName
        currentUser.email = user.email
        currentUser.age = user.age
        print ("\(currentUser.userName) has logged in. Email: \(currentUser.email). Age: \(currentUser.age)")
    }
}
 ...
 */
User.logIn(user: User(userName: "Nate", email: "Dont have one", age: 47))
